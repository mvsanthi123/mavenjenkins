...
`SERVER_PORT` environment variable to the desired value:
   ```
   SERVER_PORT=8888 java -jar target/artcifact-server.jar
   ``` 
...
1. Run the server application depending on the config profile. Below are the choices:
   * Default with no profile will use `config_file,instance_principals,resource_principal` authentication strategy
     ```bash
     java -jar target/artcifact-server.jar
     ```
   * `test` profile will use user principal via oci config file authentication
     ```bash
     java -Dconfig.profile=test -jar target/artcifact-server.jar
     ```
   * `prod` profile will use instance principal authentication
     ```bash
     java -Dconfig.profile=prod -jar target/artcifact-server.jar
     ```
...
1. Build docker image
   ```
   docker build .. -t artcifact-server -f Dockerfile
   ```
2. Run the application
   * Run from local machine using User Principal authentication. This will mount `~/.oci` as a volume in the running 
     docker container.
     ```
     docker run --user helidon --rm -p 8080:8080 -v ~/.oci:/home/helidon/.oci:ro artcifact-server:latest
     ```
   * Run from an OCI instance using Instance Principal
     ```
     docker run --user helidon --rm -p 8080:8080  artcifact-server:latest
     ```
...
[target/artcifact-server-distribution.zip](target/artcifact-server-distribution.zip)
...
```bash
$ scp target/artcifact-server-distribution.zip user@DESTINATION_HOST:/home/user
```
3. Unzip (will extract to `artcifact-server` directory) and run
```bash
$ unzip artcifact-server-distribution.zip
$ java -jar artcifact-server/artcifact-server.jar
```